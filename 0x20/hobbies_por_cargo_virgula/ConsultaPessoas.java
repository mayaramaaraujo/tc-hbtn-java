...
public class ConsultaPessoas {
    ...
    static Map<String, List<String>> obterHobbiesPorCargo(List<Pessoa> pessoas) {
        return pessoas.stream().collect(Collectors.groupingBy(Pessoa::getCargo,
                Collectors.flatMapping(pessoa -> pessoa.getHobbies().stream(), Collectors.toList())));
    }

    static Map<String, String> obterHobbiesPorCargoVirgula(List<Pessoa> pessoas) {
        return pessoas.stream().collect(Collectors.groupingBy(Pessoa::getCargo,
                Collectors.flatMapping(pessoa -> pessoa.getHobbies().stream(), Collectors.joining(","))));
    }
}
